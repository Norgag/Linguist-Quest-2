...
function loadQuestion() {
    inputLocked = false;
    el.options.classList.remove("next-layout", "centered-layout");
...
function handleAnswer(i) {
    ...
    if (i === currentQuestion.correct) {
        ...
        buttons[currentQuestion.correct].classList.add("correct-choice");
        ...
        setTimeout(() => {
            ...
            playerHealth <= 0 ? handleDeath() : showNextButton();
        }, 1000);
    }
}
...
function showNextButton() {
    el.options.innerHTML = "";
    el.options.classList.add("next-layout");
    const nextBtn = document.createElement("button");
    nextBtn.className = "next-btn";
    nextBtn.innerText = "Next Question →";
    nextBtn.onclick = loadQuestion;
    el.options.appendChild(nextBtn);
}
